...
Let's compile some summary statistics
...
Here, I import the facility birth data to normalize our MM data and find MMR. (09/07/25).

```{r}
# Using year, month, and facility id in the facility dataset to match data in patient-level df
df <- df |>
  mutate(
  ...
  drop_na(facility_code)
```


```{r}
# Import dataset
df2 <- read.csv("../data/raw/data_facility_joseph.csv")
...
# We will use the live birth counts (even though there are more missing values in that column than in the surviving infants column) because I think we can't correlate to strongly when we use data for infants born in different months.
df3 <- df2 |>
  rename(
    report_date = periodid,
    facility_code = organisationunitcode,
    live_births = MOH.711.Live.birth
  ) |>
  mutate(
    facility_code = as.integer(facility_code),
    year = year(parse_date_time(periodname, "%B %Y")),
    month = month(parse_date_time(periodname, "%B %Y"))
  ) |>
  select(
    c(
      facility_code,
...
intersect(df$facility_code, df3$facility_code)

# 97.8% of the facilities we have in the patient-level data have some sort of live births data
length(intersect(df$facility_code, df3$facility_code)) / length(unique(df$facility_code))
...
ggplot(
  ...
  geom_smooth(
    ...
    aes(x = date, y = rate, color = factor(year)),
    linewidth = 0.8,
    method = "loess",
    se = FALSE,
    span = 0.5
...
ggplot(
  ...
  labs(
    title = "Age distribution of maternal mortality<br><span style='font-size:10pt;'>Median age and interquartile range shown for each year</span>",
...
df_code_map <- df_icd_codes |>
  ...
  mutate(description = coalesce(
    description.x,
    description.y
  )) |> # coalesce grabs the values from df_icd_codes
...
cause_year_table <- df_mm_causes |>
  mutate(
    cause_label = if_else(is.na(cause_of_mortality), "Missing",
      str_to_sentence(cause_of_mortality)
    ),
    year = year(report_date)
  ) |>
  group_by(year, cause_label) |>
  ...
  mutate(
    cause_label = case_when(
      cause_label == "Covid-19" ~ "COVID-19",
      TRUE ~ cause_label
    )
  )

# Calculate column totals
year_totals <- cause_year_table |>
  summarise(across(where(is.numeric), sum))

# Create percentage table
percentage_table <- cause_year_table |>
  select(-Total) |>
  mutate(across(
    where(is.numeric),
    ~ round(100 * .x / year_totals[[cur_column()]], 1)
  )) |>
  mutate(Total = round(100 * cause_year_table$Total / sum(cause_year_table$Total), 1))

year_cols <- setdiff(names(cause_year_table), "cause_label")
combined_table <- cause_year_table |>
  mutate(across(
    all_of(year_cols),
    ~ paste0(
      get(cur_column(), pos = cause_year_table),
      " (", get(cur_column(), pos = percentage_table), "%)"
    )
  ))

# Calculate totals for the totals row
year_totals_vector <- year_totals |> unlist()
grand_total <- sum(year_totals_vector[names(year_totals_vector) != "Total"])

# Create totals row
totals_row <- tibble(cause_label = "Total") |>
  bind_cols(
    map_dfc(year_cols, function(colname) {
      val <- year_totals_vector[[colname]]
      if (colname == "Total") {
        pct <- 100.0
      } else {
        pct <- round(100 * val / grand_total, 1)
      }
      tibble(!!colname := paste0(val, " (", pct, "%)"))
    })
  )

# Sort by total count and add totals row
final_table <- combined_table |>
  mutate(Total_numeric = as.numeric(str_extract(Total, "^[0-9]+"))) |>
  arrange(desc(Total_numeric)) |>
  select(-Total_numeric) |>
  bind_rows(totals_row)

final_table |>
  knitr::kable(
    col.names = c("Cause of mortality", names(final_table)[-1]),
    caption = "Causes of maternal mortality by year",
...
# Table for export as png
final_table_ft <- final_table |>
  flextable() |>
  set_header_labels(
    cause_label = "Cause of mortality",
    Total = "Total"
  ) |>
  add_header_row(
    values = "Causes of maternal mortality by year",
    colwidths = ncol(final_table)
  ) |>
  align(align = "right", part = "body") |>
  align(align = "left", j = "cause_label", part = "body") |>
  align(align = "center", part = "header") |>
  bold(part = "header") |>
  bold(i = nrow(final_table), part = "body") |>
  border_remove() |>
  hline_top(border = fp_border_default(width = 2), part = "header") |>
  hline_bottom(border = fp_border_default(width = 2), part = "header") |>
  hline_bottom(border = fp_border_default(width = 1), part = "body") |>
  hline(i = nrow(final_table) - 1, border = fp_border_default(width = 1), part = "body") |>
  autofit() |>
  add_footer_lines("Count (% of yearly total)") |>
  bg(part = "all", bg = "white")

# final_table_ft

# Save as png
# save_as_image(final_table_ft,
#   path = "../data/figures/causes_of_maternal_mortality.png",
#   width = 12, height = 8, res = 300, expand = 10,
#   zoom = 2,
#   background = "white"
# )
```
...
```{r}
df_graph_time <- df_mm_causes |>
  filter(cause_of_mortality == "pregnancy, childbirth or the puerperium") |>
  group_by(
    month,
    year,
    facility_code
