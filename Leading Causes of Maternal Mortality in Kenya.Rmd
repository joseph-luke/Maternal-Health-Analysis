...
### Leading causes of maternal mortlaity: clinical perspective
...
#   temp_extracted = str_extract(direct_cause_of_death_code, "^[^ /\\\\&,]+"),
#   final_code = if_else(is.na(temp_extracted), code, temp_extracted),
#   final_code = str_to_lower(final_code),
#   final_code = str_replace_all(final_code, "[^a-z0-9]", "")  # remove non-alphanumeric
# ) |>
#   filter(!is.na(final_code)) |>
#   pull(final_code) |> # Grab elements from column using pull
#   unique()
...
df_icd_10 <- read_xlsx("../data/raw/icd_10_codes.xlsx") |>
  clean_names() |>
  rename(
...
# Import and clean ICD 11 codes
df_icd_11 <- read_csv("../data/raw/icd_11_codes.csv",
                           col_names = FALSE) |>
  clean_names() |>
...
df_icd_codes <- bind_rows(df_icd_10,
                          df_icd_11)

# Export for ML in python:
write_csv(df_icd_codes,
          "../data/raw/icd_codes.csv")
write_csv(data.frame(code = as.character(unique_codes)),
          "../data/raw/dataset_codes.csv") # make sure to open as str in python
